...
// Sodimac PE migrated to Next.js (Falabella stack) in May 2026.
// The offers hub exposes __NEXT_DATA__ with structure identical to Falabella.
// /search?Ntt=... URLs are safe — never redirect to home.
const CATEGORIAS_BASE = [
  `${BASE}/sodimac-pe/seleccion/ofertas-sodimac`,
  `${BASE}/sodimac-pe/search?Ntt=herramientas`,
...
function extractItems(html) {
  ...
  for (const item of results) {
    try {
      ...
      if (!current || current <= 0 || !original || original <= current) continue;
