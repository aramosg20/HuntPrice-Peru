...
// Mobile Peru UA: tiendas peruanas sirven HTML más simple y son menos estrictas con bots
const HEADERS = {
...
// VTEX IO intelligent-search fallback (works on IO-migrated stores)
...
async function ping(state, extra = '') {
  ...
  if (!key) return;
  try { await fetch(`https://cronitor.link/p/${key}/huntprice-scraper-promart?state=${state}${extra}`); } catch (_) {}
}
...
async function scrape() {
  ...
  // Attempt 1: Legacy VTEX catalog API (most Promart PE stores still use this)
  ...
  // Attempt 2: VTEX IO intelligent-search (if legacy returned nothing)
