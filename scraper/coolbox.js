...
// Coolbox PE runs on VTEX IO (React/GraphQL frontend, same Hybris catalog backend as Promart/PlazaVea)
// The Shopify approach was wrong — /collections/*.json returns HTML (VTEX IO SPA shell)
// VTEX catalog REST API at /api/catalog_system/pub/products/search/ works and returns 206 Partial Content
...
async function ping(state, extra = '') {
  ...
  if (!key) return;
  try { await fetch(`https://cronitor.link/p/${key}/huntprice-scraper-coolbox?state=${state}${extra}`); } catch (_) {}
}
...
function guessCategory(name) {
  ...
  if (/audifonos|auricular|bluetooth|airpods|headphone/.test(n)) return 'Electrónica';
